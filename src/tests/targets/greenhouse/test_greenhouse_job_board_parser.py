...
class TestGreenhouseBoardParser(unittest.TestCase):
    ...
    def test_extract_greenhouse_job_info(self, mock_get):
        with open('tests/targets/greenhouse/fixtures/board_page_with_several_placements.html', 'r') as file:
        ...
        parser = GreenhouseBoardParser(self.logger)

        # Call the method under test
        url = "https://boards.greenhouse.io/remotecom"
        job_info = parser.extract_greenhouse_job_info(url)

        assert len(job_info) == 8
        assert job_info[1] == {'url': 'https://boards.greenhouse.io/fixture_board_company/jobs/4362194005',

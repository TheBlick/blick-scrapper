...
class GreenhouseBoardParser:
    ...
    def extract_greenhouse_job_info(self, url):
        ...
        if response.status_code != 200:
            self.__logger.info(f"Failed to retrieve the webpage: Status code {response.status_code}")
            return []
        ...
        for job_link in job_links:
            ...
            job_location = location_tag.text.strip() if location_tag else 'Location not specified'

    ...
    def classify_urls_to_dict(self, url_list):
        ...
        for url in url_list:
            ...
            if self.__match_url_to_category(url=url, pattern=JOB_BOARD_REGEX_PATTERN):
                for job_board_dict in self.job_board_parser.extract_greenhouse_job_info(
                    url
                ):
                    model = JobPostingFromBoard(
                        url=job_board_dict.get("url"),
                        origin_url=url,
                        title=job_board_dict.get("title"),
                        location=job_board_dict.get("location"),
                    )
